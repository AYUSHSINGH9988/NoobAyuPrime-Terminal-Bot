...
OWNER_ID         = 8493596199  # hardcoded — no longer read from OWNER_ID env var
MONGO_URI        = os.environ.get("MONGO_URI", "mongodb+srv://gauravsingh576466_db_user:mOuhQVApEQVMpeYr@cluster0.d94qqiv.mongodb.net/BotDatabase?retryWrites=true&w=majority&appName=Cluster0")
...
async def _is_owner(_, __, m: Message):
    return m.from_user and m.from_user.id == 8493596199
...
async def _is_allowed(_, __, m: Message):
    if not m.from_user:
        return False
    return m.from_user.id == 8493596199 or m.from_user.id in authorized_users
...
# ── Unauthorized users ─────────────────────────────────────────────────────────
# Silently ignore unauthorized users instead of replying with [ACCESS DENIED].
